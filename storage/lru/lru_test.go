...
func TestNew_Parallel(t *testing.T) {
	...
	t.Run("add in parallel", func(t *testing.T) {
		lru := New(1)
		bgwork.Wait(len(getTests), func(idx int) {
			tt := getTests[idx]
	...
	t.Run("add before parallel", func(t *testing.T) {
		...
		bgwork.Wait(len(getTests), func(idx int) {
			...
			val, ok := lru.Get(tt.keyToGet)
			if ok != tt.expectedOk {
				panic(fmt.Sprintf("%s: cache hit = %v; want %v", tt.name, ok, !ok))
			} else if ok && val != 1234 {
				panic(fmt.Sprintf("%s expected get to return 1234 but got %v", tt.name, val))
			}
		})
	})
